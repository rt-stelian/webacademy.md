(() => {
    ...
    const changeLang = event => {
        ...
        url = url.replace('/en/', '/' + selectedLang + '/');
    ...
    freeBtn.addEventListener("click", (ev => {
        ev.preventDefault();
        formTitle.textContent = "Get a free lesson!";
        popupOpen();
    }));
    planTitle.forEach((el => {
        el.addEventListener("click", (ev => {
            ...
            if (ev.target.classList.contains("plan-column__btn")) {
                ...
                formTitle.textContent = `Your choose is ${title}`;
                popupOpen();
            }
        }));
    }));
    ...
    const validators = {
        ...
        nameInput(el, warning) {
            return warning.textContent = `The field "${el.previousElementSibling.firstChild.textContent.trim()}" must contain at least 2 words!`;
        },
        otherInputs(el, warning) {
            return warning.textContent = `The field "${el.previousElementSibling.firstChild.textContent.trim()}" is required!`;
        },
    ...
    function checkWarning() {
        ...
        warning.textContent = `Please select at least one item!`;
    }
