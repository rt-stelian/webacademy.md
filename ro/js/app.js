(() => {
    ...
    function menuInit() {
        ...
        document.querySelectorAll(".header__nav-items");
        if (menuBurger) document.addEventListener("click", (function (e) {
            if (bodyLockStatus && e.target.closest(".icon-menu")) {
                bodyLockToggle();
                menuBurger.classList.toggle("menu-open");
                headerNav.classList.toggle("active");
            }
            if (bodyLockStatus && e.target.closest(".header__nav-items")) {
                bodyLockToggle();
    ...
    const langBtn = document.querySelector(".lang-switcher__button");
    const langMenu = document.querySelector(".lang-switcher__menu");
    const langLinks = document.querySelectorAll(".lang-switcher__lang-link");
    const getPageLang = () => document.documentElement.getAttribute("lang");
    const setBtnLangName = () => {
        const pageLang = getPageLang();
        langLinks.forEach((({ style, textContent, dataset }) => {
            style.display = "block";
            const langName = dataset.lang;
            if (langName === pageLang) {
                langBtn.textContent = textContent;
                style.display = "none";
            }
        }));
    };
    ...
    const changeLang = event => {
        ...
        url = url.replace('/ro/', '/' + selectedLang + '/');
        window.location.href = url
        setBtnLangName();
        langMenu.classList.remove("active");
    };
    const langLenght = () => {
        const isMobile = window.innerWidth < 767.89;
        langLinks.forEach((el => {
            const langShort = el.dataset.shortName;
            const langFull = el.dataset.fullName;
            isMobile ? el.textContent = langShort : el.textContent = langFull;
            setBtnLangName();
        }));
    };
    window.addEventListener("resize", langLenght);
    langBtn.addEventListener("click", setLang);
    langLinks.forEach((link => {
        link.addEventListener("click", changeLang);
    }));
    document.addEventListener("click", (event => {
        if (!event.target.closest(".lang-switcher__button") && langMenu.classList.contains("active")) langMenu.classList.remove("active");
    }));
    const initLanguage = () => {
    ...
    window.addEventListener("scroll", (() => {
        if (window.pageYOffset > 50) header.classList.add("header-fixed"); else header.classList.remove("header-fixed");
    }));
    const body = document.querySelector("body");
    const freeBtn = document.querySelector(".free-lesson__btn");
    const popup = document.querySelector(".popup");
    const popupContent = document.querySelector(".popup__content");
    const formTitle = document.querySelector(".popup__title");
    const planTitle = document.querySelectorAll(".plan-column");
    freeBtn.addEventListener("click", (ev => {
        ev.preventDefault();
        formTitle.textContent = "Obține o lecție gratuită!";
        popupOpen();
    }));
    planTitle.forEach((el => {
        el.addEventListener("click", (ev => {
            ev.preventDefault();
            if (ev.target.classList.contains("plan-column__btn")) {
                const title = el.querySelector(".plan-column__title").textContent;
                formTitle.textContent = `Ai ales ${title}`;
                popupOpen();
            }
        }));
    }));
    ...
    function popupOpen() {
        const padding = window.innerWidth - document.documentElement.offsetWidth + "px";
    ...
    function popupClose(ev) {
        ev.stopPropagation();
        if (ev.target.classList.contains("popup__close") || ev.target.classList.contains("popup__wrapper")) {
    ...
    const reqInput = document.querySelectorAll("[data-required]");
    script_form.addEventListener("submit", (ev => {
        ev.preventDefault();
        formValidate();
        removeCheckWarning();
        checkBoxValidate();
    ...
    function formValidate() {
        reqInput.forEach((el => {
            const inputId = el.id;
            const id1 = formReqInputs.inputName.id;
            const id2 = formReqInputs.inputTel.id;
            const id3 = formReqInputs.inputEmail.id;
            validators.removeReq(el, inputId, id1, id2, id3);
            validators.warningReq(el, inputId, id1, id2, id3);
        }));
    }
    ...
    function clearForm() {
        ...
        if (!required.length) reqInput.forEach((el => {
            const inputId = el.id;
            const id1 = formReqInputs.inputName.id;
            const id2 = formReqInputs.inputTel.id;
            const id3 = formReqInputs.inputEmail.id;
    ...
    const validators = {
        removeReq(el, elemId, id1, id2, id3) {
            el.style.borderColor = "";
            el.nextElementSibling ? el.nextElementSibling.remove() : null;
            el.addEventListener("input", (() => {
                el.style.borderColor = "";
                el.nextElementSibling ? el.nextElementSibling.remove() : null;
                this.warningReq(el, elemId, id1, id2, id3);
            }));
        },
        addReq(el, fn) {
        ...
        nameInput(el, warning) {
            return warning.textContent = `Câmpul "${el.previousElementSibling.firstChild.textContent.trim()}" trebuie să conțină cel puțin 2 cuvinte!`;
        },
        otherInputs(el, warning) {
            return warning.textContent = `Câmpul "${el.previousElementSibling.firstChild.textContent.trim()}" este obligatoriu!`;
        },
        warningReq(el, elemId, id1, id2, id3) {
            if (id1) if (elemId === id1) if (el.value.split(" ").length < 2) this.addReq(el, this.nameInput);
            if (id2) if (elemId === id2 && el.value.length <= 9) this.addReq(el, this.otherInputs);
            if (id3) if (elemId === id3 && el.value.length <= 9) this.addReq(el, this.otherInputs);
        }
    };
    const checkTitle = document.querySelector(".checkbox > .popup__input-title");
    ...
    function checkBoxValidate() {
        ...
        const checkbox = document.querySelectorAll('.checkbox input[type="checkbox"]:checked');
        if (checkbox.length < 1) checkWarning();
        if (checkbox.length >= 1) removeCheckWarning();
    }
    function checkWarning() {
        checkboxes.forEach((el => {
            el.style.boxShadow = "inset  0 0 3px red";
        }));
        const warning = document.createElement("span");
        warning.classList.add("required");
        ...
        checkTitle.style.marginBottom = "5px";
        checkTitle.insertAdjacentElement("afterend", warning);
        warning.textContent = `Vă rugăm să bifați cel puțin un element!`;
    }
    function removeCheckWarning() {
        checkTitle.style = `/* margin-bottom: 5px */`;
        const warningText = document.querySelector(".checkbox >  span");
        warningText ? warningText.remove() : null;
        checkboxes.forEach((el => {
            el.style.boxShadow = "";
        }));
    }
