...
(() => {
    ...
    const changeLang = event => {
        ...
        url = url.replace('/ru/', '/' + selectedLang + '/');
    ...
    freeBtn.addEventListener("click", (ev => {
        ev.preventDefault();
        formTitle.textContent = "Хочу бесплатный урок!";
        popupOpen();
    }));
    planTitle.forEach((el => {
        el.addEventListener("click", (ev => {
            ...
            if (ev.target.classList.contains("plan-column__btn")) {
                ...
                formTitle.textContent = `Вы выбрали ${title}`;
                popupOpen();
            }
        }));
    }));
    ...
    const validators = {
        ...
        nameInput(el, warning) {
            return warning.textContent = `Поле "${el.previousElementSibling.firstChild.textContent.trim()}" должно содержать как минимум 2 слова!`;
        },
        otherInputs(el, warning) {
            return warning.textContent = `Поле "${el.previousElementSibling.firstChild.textContent.trim()}" обязательно к заполнению!`;
        },
    ...
    function checkWarning() {
        ...
        warning.textContent = `Пожалуйста, выберите хотя бы один вариант!`;
    }
